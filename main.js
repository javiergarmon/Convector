...
var rsync = function( callback ){
  ...
  if( firstSync ){
    firstSync = false;
    notifier.notify( { 'title': 'Convector', 'message': 'First sync, it can take a while...' }, function(){} );
    console.log( 'First sync, it can take a while...'.yellow );
  }else{
    notifier.notify( { 'title': 'Convector', 'message': 'Syncing...' }, function(){} );
    console.log( 'Syncing...'.yellow );
  }

  execFile( 'rsync', [ '-avrP', '--exclude=".*"', SOURCE, DESTINY ], { cwd : process.cwd, maxBuffer : 1024 * 1024 * 1024 }, function( error, stdout, stderr ){
    ...
    notifier.notify( { 'title': 'Convector', 'message': 'Uploaded ' + count + ( count === 1 ? ' file' : ' files' ) }, function(){} );
